...
function palabras(action, id, text) {
    if(typeof(id) === "undefined") id = 0;
    
    switch (action) {
        case 'formNew':
            $.ajax({
                url: "../classes/class_word.php",
                type: "post",
                data: {action: "formNew"},
        ...
        case 'formEdit':
            $.dialog({
                ...
                columnClass: 'col-7',
                content: `url: ../classes/class_word.php?action=${action}&id_palabra_to_update=${id}`, // Aquí le pasamos el ID del usuario
        ...
        case 'delete':
            $.confirm({
                'title': '¿Seguro que desea borrar la palabra?',
                'type': 'red',
                'buttons': {
                    confirm: function(){
                        $.ajax({
                            url: "../classes/class_word.php",
                            type: "post",
                            data: {action: "delete", id_palabra: id},
                            success: function(htmlResponse){
